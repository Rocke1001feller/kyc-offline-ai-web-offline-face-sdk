...
async function loadLivenessModel() {
  ...
  await createSessionWithFallback("../model/fr_liveness.onnx", {executionProviders: ['wasm']})
      .then((session) => {
...
async function predictLivenessBase64(session, base64Image, bbox) {
  let image = new Image()
...
export {loadLivenessModel, predictLiveness, predictLivenessBase64}
